/*-----------------------------------------------------------
 ...
 where the arguments are as follows:
  ...
  converge specifies the convergence criterion for the macro _and_ . 
           for nonlinear regression (using NLINMETHOD=1). The default is 
           CONVERGE=1e-8 1e-1.                                       
  ...
  maxit    specifies the maximum number of iterations for the 
           ...
           (using NLINMETHOD=1).  The default is MAXIT=30 20, which means that maximum
           # of iterations is 30 for NLINMIX iterations and 20 for NLIN iterations.            
  ...
  nlinmethod
           ...
           By default is = 1 and iterative calls to PROC MIXED are used.
           If nlinmethod=2 and nlmem is blank (non-IML execution mode) 
             then PROC NLIN is used.
           If nlinmethod=2 and nlmem is _not_  blank(IML execution mode)
              then NLPLM is used instead of PROC NLIN.
  ...
  neq_constraints
           ...
           b1>=0; b2-b1>=0; 1< b3<=10; 2*b1 +b3<5
...
%if (&saveb = 0 or &nlinmethod ne 1) %then %do;
  ...
  %usr_debug(101); /* DATASETS:  &outdata _beta  */ 
  %end;
...
%do %while(&ni <= &maxiter);
   ...
   %if (&ni ne 0) %then %do;
      ...
      %if (&cov_crit=0) %then %do;
         %let _trace=&_trace NOT; 
         %if (&nu) %then %let temp=(COVP not used);
             %else %let temp=;
         %let crit= &crit;
      %end;
...
%macro nlinmix(data=,modinit=,model=,weight=,derivs=, tol=1e-5,
   parms=,stmts=,expand=zero,converge= ,maxit=,switch=0,
   append=,procopt=,nlinopt=,nlinstmt=,outdata=_nlinmix,options=,
   paramfixed =,divisor=0.7, postpd=, postmixed=, nlinmethod=1,
...
%else %do;
  ...
  %end;
   ...
   %let default_converge=1e-8 1e-1;
   %let converge1=%scan(&converge,1,' ');
   %let converge2=%scan(&converge,2,' ');
   %if (%length(&converge1)=0 | &converge1=.) %then 
        %let converge1=%scan(&default_converge,1,' ');
   %if (%length(&converge2)=0 | &converge2=.) %then 
        %let converge2=%scan(&default_converge,2,' ');

   %let default_maxit=30 20;
   %let maxit1=%scan(&default_maxit,1,' ');
   %let maxit2=%scan(&default_maxit,2,' ');
   %if (%length(&maxit1)=0 | &maxit1=.) %then 
        %let maxit1=%scan(&default_maxit,1,' ');
   %if (%length(&maxit2)=0 | &maxit2=.) %then 
        %let maxit2=%scan(&default_maxit,2,' ');
