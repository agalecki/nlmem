/* Posted on the web on September, 16. 2002 */
 ...
 *print _nobs;
  optn= _nobs ||{5};
  call nlplm(rc,_bnlin,"_objf",_bx,optn) jac="_dobjf";
  print _bnlin;
%mend  default_nlin;
 ...
 *print _nobs;
  optn= _nobs ||{0};
  call nlpfdd(_objfv,g,h,"_objf",_bx,optn) jac="_dobjf";
  print _objvf _dobjf;
  ...
  *print 'res_dr2' _u;
  ...
  &pseudoresp = _observedy - predv;
   ...
   _dbu= _derivs(_bu);
      ...
      %if (&nb) %then %do; 
            indx={&adjloop};
            &pseudoresp= &pseudoresp + _dbu[,indx]*_b[indx];
      %end;
...
%macro _pseudoder;
start _pseudoder(_bx) global(_nb,_nu,_call, _nobs,_observedy,_ni,_globalb);
  _b=_globalb;
   v={&numadjset};
  _b[v]=_bx;
...
start _objf(_bx) global( _call, _nobs, _observedy, _dobjf,_ni,
                       _globalb
                      );
  _b=_globalb;
   v={&numadjset};
...
start _dobjf(_bx) global (_dobjf,_ni,_globalb);
  _b=_globalb;
   v={&numadjset};
  _b[v]=_bx;  
 return(_dobjf);
...
%macro pseudoder_nlin;
 /* Invoked by <nlin> macro */
 proc iml;
 /*Creates _nlinmx2 and  outdata dataset */
   %pseudoder_common
   use _bnlin var {&adjset} ;
       read next 1;
      _bnlin=%aux(&adjset,||);
   close _bnlin;
   free &fixed;
